...
<div id="print-modal" class="modal fade large" tabindex="-1" role="dialog" aria-labelledby="myPrintModalLabel" aria-hidden="true">
    ...
    <div class="modal-body" >
        ...
        <div style="border-bottom:thin solid #ccc;" class="row clearfix">
            ...
            <div class="col-md-2">
                {{ Former::text('label_cell_height','Label height')->value('150')->id('label_cell_height')->class('form-control input-sm') }}
            ...
            <div class="col-md-2">
                {{ Former::text('label_offset_left','Page left offset')->value('40')->id('label_offset_left')->class('form-control input-sm') }}
                {{ Former::text('label_offset_top','Page top offset')->value('20')->id('label_offset_top')->class('form-control input-sm') }}
            </div>
            <div class="col-md-2">
        ...
        <iframe id="label_frame" name="label_frame" width="100%" height="80%"
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#label_refresh').on('click',function(){
            ...
            var src = '{{ URL::to('rack/printlabel')}}/' + sessionname + '/' + col + ':' + res + ':' + cell_width + ':' + cell_height + ':' + margin_right + ':' + margin_bottom + ':' + font_size + ':' + code_type + ':' + offset_left + ':' + offset_top;
                ...
                $.post('{{ URL::to('ajax/sessionsave')}}',
                    ...
                    function(data){
                        if(data.result == 'OK'){
                            ...
                            var offset_left = $('#label_offset_left').val();
                            var offset_top = $('#label_offset_top').val();
                            var src = '{{ URL::to('rack/printlabel')}}/' + data.sessionname + '/' + col + ':' + res + ':' + cell_width + ':' + cell_height + ':' + margin_right + ':' + margin_bottom + ':' + font_size + ':' + code_type + ':' + offset_left + ':' + offset_top;
                            $('#label_frame').attr('src',src);
